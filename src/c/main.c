...
//   time     76px  framed box, DSEG7 44px digits over ghost segments,
...
static GFont s_font_dseg44, s_font_dseg18, s_font_tech18, s_font_tech11,
             s_font_tech8;
...
static void stipple_create(void) {
  s_stipple = gbitmap_create_blank_with_palette(
      ...
      (GColor[]){GColorClear, T->mute}, true);
  if (!s_stipple) return;
...
static void time_update_proc(Layer *layer, GContext *ctx) {
  ...
  GSize d2 = text_size("88", s_font_dseg44);       // HH or MM block
  GSize colon = text_size(":", s_font_dseg44);
  ...
  draw_text(ctx, "88", s_font_dseg44, T->ghost, GRect(x, y, d2.w + 2, d2.h + 2));
  draw_text(ctx, ":", s_font_dseg44, T->ghost,
            GRect(x + d2.w, y, colon.w + 2, d2.h + 2));
  draw_text(ctx, "88", s_font_dseg44, T->ghost,
            GRect(x + d2.w + colon.w, y, d2.w + 2, d2.h + 2));

  // Lit digits. Colon blinks at the design's 0.5 Hz cycle.
  draw_text(ctx, hh, s_font_dseg44, T->ink, GRect(x, y, d2.w + 2, d2.h + 2));
  if (s_now.tm_sec % 2 == 0) {
    draw_text(ctx, ":", s_font_dseg44, T->ink,
              GRect(x + d2.w, y, colon.w + 2, d2.h + 2));
  }
  draw_text(ctx, mm, s_font_dseg44, T->ink,
            GRect(x + d2.w + colon.w, y, d2.w + 2, d2.h + 2));
...
static void window_load(Window *window) {
  ...
  s_font_dseg44 = fonts_load_custom_font(
      resource_get_handle(RESOURCE_ID_FONT_DSEG7_44));
  s_font_dseg18 = fonts_load_custom_font(
      resource_get_handle(RESOURCE_ID_FONT_DSEG7_18));
...
static void window_unload(Window *window) {
  ...
  fonts_unload_custom_font(s_font_dseg44);
